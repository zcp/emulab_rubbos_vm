...
static const char *log_request_duration_microseconds(request_rec *r, char *a)
{
    ...
    if(r->connector_start_time == 0)
    {
	//return apr_psprintf(r->pool, "%" APR_TIME_T_FMT,
        //                (current_time - r->request_time));
        return apr_psprintf(r->pool, "ST(us)=%" APR_TIME_T_FMT " ET(us)=%" APR_TIME_T_FMT " RT(us)=%" APR_TIME_T_FMT,
                        (r->request_time), (current_time),(current_time - r->request_time));
    }
//    return apr_psprintf(r->pool, "%" APR_TIME_T_FMT " %" APR_TIME_T_FMT " %" APR_TIME_T_FMT " %" APR_TIME_T_FMT,
//                        (current_time - r->request_time), (r->connector_start_time - r->request_time),
//				(r->connector_end_time - r->connector_start_time), (current_time - r->connector_end_time));
    return apr_psprintf(r->pool, "ST(us)=%" APR_TIME_T_FMT " ET(us=)%" APR_TIME_T_FMT " RT(us)=%" APR_TIME_T_FMT " TCST(us)=%" APR_TIME_T_FMT " TCET(us)=%" APR_TIME_T_FMT " TCRT(us)=%" APR_TIME_T_FMT,
                        (r->request_time), (current_time), (current_time - r->request_time), (r->connector_start_time),
                                (r->connector_end_time), (r->connector_end_time - r->connector_start_time));
}
