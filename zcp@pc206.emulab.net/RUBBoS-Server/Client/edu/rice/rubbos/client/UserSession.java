...
public class UserSession extends Thread
{
  ...
  private String callHTTPServer(URL url)
  {
    ...
    //String		logMessage = "Thread " + this.getName();
    String logMessage = "Request :" + url;
    String urlID = url.toString();
    int urlIndex = urlID.indexOf("?urlID=");
    urlIndex += 7;
    int urlEndIndex = urlID.indexOf("&", urlIndex);
    if(urlEndIndex == -1)
	urlID = urlID.substring(urlIndex);
    else
        urlID = urlID.substring(urlIndex, urlEndIndex);
    ...
    logMessage += " STimeStamp(ms)" + stimestamp;
    stime = System.nanoTime();
    //logMessage = "Yasu0,Thread:" + this.getName() + ",url:" + url + ",st:" + stime;
    logMessage += " GetConST(nano)=" + stime;
	...
	//ChienAn
      ...
      logMessage += " GetConET(nano)=" + tmp_time;
	...
	tmp_time = System.nanoTime();
	logMessage += " CloseCon(nano)=" + tmp_time;
	...
	tmp_time = System.nanoTime();
	logMessage += " LDImageST(nano)=" + tmp_time;

      num++;
      URL imageURL = urlGen.genericHTMLFile((String)images.elementAt(0)+ "?urlID=" + urlID);
      try
      {
	...
	//ChienAn
      ...
      //logMessage += ",t4:" + tmp_time;
        tmp_time = System.nanoTime();
        logMessage += " LDImageET(nano)=" + tmp_time;
    }
    ...
    //long rt = etime - stime;
    long rt = (etime - stime)/1000;
    //if (rt > 3000) {
    //	logMessage += ",et:" + etime + ",rt:" + rt;
        logMessage += " ET(nano)=" + etime + " RT(us)=" + rt;
  ...
  public void run()
  {
	...
	//ChienAn change to record microsecond
        time = System.currentTimeMillis();
        //long nanosecond1 = System.nanoTime();
        lastHTMLReply = callHTTPServer(lastURL);
	//long nanosecond2 = System.nanoTime();
	//long microResponseTime = (nanosecond2 - nanosecond1)/1000;
	// Yasu:
        time2 = System.currentTimeMillis();
        //System.err.println("Request :" + lastURL + " ST(nano)=" + nanosecond1 + " ET(nano)=" + nanosecond2 + " RT(us)=" + microResponseTime);
	//time = nanosecond1 / 1000000;
	//time2 = nanosecond2 / 1000000;
